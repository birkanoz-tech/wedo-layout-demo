...
export function populate2DConveyorCADGeometry(group, pathData, widthM = 0.105, assyName = null) {
    ...
    bendDataMap.forEach((bend) => {
        const { isLeft, R, norm1, cArc, pBendStart, pArcStart, pArcEnd, pBendEnd, u1, u2 } = bend;
        const norm2 = isLeft ? new THREE.Vector3(-u2.y, u2.x, 0) : new THREE.Vector3(u2.y, -u2.x, 0);

        const R_in = Math.max(0.01, R - halfW);
        const R_out = R + halfW;

        // A. Giriş Flanşı / Birleşim Çizgisi (Seam line across conveyor at pBendStart)
        const inLeft1 = pBendStart.clone().addScaledVector(norm1, isLeft ? halfW : -halfW);
        const inRight1 = pBendStart.clone().addScaledVector(norm1, isLeft ? -halfW : halfW);
        boundaryLinePts.push(inLeft1, inRight1);

        // B. 200mm Giriş Düzlüğü
        const inLeft2 = pArcStart.clone().addScaledVector(norm1, isLeft ? halfW : -halfW);
        const inRight2 = pArcStart.clone().addScaledVector(norm1, isLeft ? -halfW : halfW);
        boundaryLinePts.push(inLeft1, inLeft2);
        boundaryLinePts.push(inRight1, inRight2);
        axisLinePts.push(pBendStart, pArcStart);

        // C. Kavisli Yay (Radius Arc): Orta çizgi R, iç çizgi R-halfW, dış çizgi R+halfW
        ...
        const arcSteps = 24;
        let prevInner = inLeft2;
        let prevOuter = inRight2;
        ...
        for (let s = 1; s <= arcSteps; s++) {
            ...
            const currInner = new THREE.Vector3(cArc.x + cosA * R_in, cArc.y + sinA * R_in, zPos);
            const currOuter = new THREE.Vector3(cArc.x + cosA * R_out, cArc.y + sinA * R_out, zPos);

            boundaryLinePts.push(prevInner, currInner);
            boundaryLinePts.push(prevOuter, currOuter);
        ...
        const outLeft2 = pBendEnd.clone().addScaledVector(norm2, isLeft ? halfW : -halfW);
        const outRight2 = pBendEnd.clone().addScaledVector(norm2, isLeft ? -halfW : halfW);

        boundaryLinePts.push(prevInner, outLeft2);
        boundaryLinePts.push(prevOuter, outRight2);
